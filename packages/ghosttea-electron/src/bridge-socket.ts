  ...
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(
      () => {
        ...
        settled = true;
        socket.destroy();
        reject(new Error(`ghosttead connection timed out during authentication at ${path}`));
      },
    ...
    socket.on("close", () => {
      if (!settled) {
        settled = true;
        clearTimeout(timeout);
        reject(new Error(`ghosttead connection closed during authentication at ${path}`));
      } else if (authenticated) {
        onDisconnect(new Error(`ghosttead connection closed at ${path}`));
      }
    });
    ...
    const drainPackets = (): void => {
      ...
      try {
        while (buffered.length >= 4) {
          ...
          if (length > limit) {
            socket.destroy(new Error("ghosttead packet exceeds bridge quota"));
            return;
          }
          ...
          if (!authenticated) {
            if (body.toString() !== "ok") {
              ...
              clearTimeout(timeout);
              socket.destroy();
              reject(new Error("ghosttead authentication failed"));
              return;
            }
            authenticated = true;
            settled = true;
