      ...
      (bytes) => {
        try {
          ...
          controlPort.postMessage({
            requestId: 0,
            type: "bridge-error",
            message: `Invalid ghosttead response: ${error.message}`,
          });
          controlSocket?.destroy();
        }
      },
      ...
      (bytes) => {
        try {
          ...
          if (
            ...
            throw new Error("ghosttead sent an invalid frame-channel message");
          }
    ...
    try {
      ...
      if (
        ...
        if (capabilities) {
          ...
          // before ghosttead can deliver the subscription's first frame.
