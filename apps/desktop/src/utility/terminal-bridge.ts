  ...
  return new Promise((resolve, reject) => {
    ...
    socket.on("data", (chunk) => {
      ...
      while (buffered.length >= 4) {
        ...
        if (!authenticated) {
          if (body.toString() !== "ok") socket.destroy(new Error("terminald authentication failed"));
          authenticated = true;
...
async function attachRenderer(rawData: unknown, ports: Electron.MessagePortMain[]): Promise<void> {
  const data = rawData as { type: string; connection: Connection };
  const [controlPort, framePort] = ports;
  if (data.type !== "connect" || !controlPort || !framePort) throw new Error("invalid bridge bootstrap");

  const controlSocket = await connectSocket(data.connection.controlSocket, data.connection.authToken, MAX_CONTROL_BYTES, (bytes) => {
    controlPort.postMessage(JSON.parse(bytes.toString("utf8")));
  });
  const frameSocket = await connectSocket(data.connection.frameSocket, data.connection.authToken, MAX_FRAME_BYTES, (bytes) => {
