...
export function ghosttySourceIdentity() {
  ...
  return {
    ...
    buildRecipe: {
      ...
      // Promotion/trust policy does not affect generated bytes. Keep it in
      // the lock, but exclude it from the content-producing recipe identity.
      targets: Object.fromEntries(
        Object.entries(lock.targets).map(([target, config]) => [
          target,
          Object.fromEntries(Object.entries(config).filter(([key]) => key !== "reproducible")),
        ]),
      ),
    },
  };
