...
| Cancellation   | Cancel auth/connect/handshake/read            | Required                  | One-shot plus 16 auth and 32 handshake cycles pass; Wi-Fi route-loss cancellation takes 23 ms on iPhone            | Repeat against launch server and background suspension   |
| Networking     | Wi-Fi/cellular transition and reconnect UX    | Required product behavior | LAN route loss cancels promptly on iPhone; a fresh connection passes after Wi-Fi restoration                       | Implement product path-state and reconnect orchestration |
