...
| -------------- | --------------------------------------------- | ------------------------- | ------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------- |
| Authentication | Password                                      | Required                  | Nonblocking Swift adapter passes on macOS and the iPhone fixture                                                   | Repeat against launch-server sample                                           |
| Authentication | Ed25519 public key                            | Required                  | Nonblocking unencrypted and encrypted keys pass                                                                    | Connect opaque Keychain reference and protected temporary materialization     |
...
| Authentication | Encrypted private-key loading                 | Required                  | Correct passphrase passes; incorrect one is rejected; Keychain policy is defined                                   | Replace long-lived string/path configuration with async credential resolution |
