...
| Authentication | Ed25519 public key                            | Required                  | Opaque resolver passes with protected private-key materialization and OpenSSL-derived public key; no key paths persist | Exercise Keychain-backed public key on device and launch-server sample |
...
| Authentication | Encrypted private-key loading                 | Required                  | Counted-byte opaque resolver passes; wrong passphrase rejects; protected file is removed on every path                 | Exercise Keychain-backed flow on device and launch-server sample       |
...
| Authentication | Keyboard-interactive, multiple prompts        | Required for common 2FA   | Zero-prompt, PAM info text, and mixed echo/no-echo rounds pass                                                         | Add nonempty protocol name/instruction server fixture                  |
