...
use ghosttea::{
    ...
    RemoteEndedReason, RemoteHostSummary, RemoteLifecycleChanged, RemoteLifecycleState,
    RemoteReplica, RemoteResize, RemoteSelection, RemoteSessionLifecycle, RemoteSessionOpen,
    RemoteTerminalRuntime, RemoteViewRecord, RemoteViewState, RemoteViewStateChanged, Session,
    SessionRegistry as Registry, SessionSummary, TerminalMesh, TerminalPresentationConfig,
    ViewAccess,
    tunnel_protocol::{
...
impl SessionLifecycle {
    ...
    fn publish(&self, state: &LifecycleState) {
        let _ = self.lifecycle_tx.send(RemoteLifecycleChanged {
            ...
            // Phase 1 has no reconnect engine, so no retry is ever scheduled.
            next_retry_ms: None,
    ...
    /// be written.
    fn begin_view_attempt(&self, local_view_id: &str) -> String {
        let mut state = self.state.lock().unwrap();
    ...
    fn commit_disconnect(&self, generation: u64, incarnation: u64) -> bool {
        ...
        // Phase 1 has no auto-reconnect engine, so Suspended is the resting
        // state with manual retry armed rather than a stop on a backoff clock.
        self.advance(&mut state, RemoteLifecycleState::Suspended, None);
        true
    }
}
...
async fn await_first_snapshot(mut synchronized: tokio::sync::watch::Receiver<bool>) -> Result<()> {
...
impl MeshRuntime {
    ...
    pub async fn attach_view(&self, session_id: &str, view_id: &str) -> Result<RemoteAttachment> {
        ...
        let outcome = match self.attach_view_attempt(&remote, view_id).await {
            ...
            Err(AttachFailure::Failed(first_error)) => {
                ...
                match self.attach_view_attempt(&remote, view_id).await {
                    Ok(outcome) => Ok(outcome),
                    Err(AttachFailure::Ended(reason)) => {
                        lifecycle.commit_ended(reason);
                        bail!("remote terminal session ended: {}", reason.as_str());
        ...
        if resuming && lifecycle.state_kind() != RemoteLifecycleState::Opening {
            ...
            await_first_snapshot(outcome.synchronized).await?;
        }
    ...
    async fn attach_view_attempt(
        ...
        let wire_view_id = remote.lifecycle.begin_view_attempt(local_view_id);
        ...
        tokio::spawn(async move {
            ...
            // Carries the incarnation and generation it acted for; the owner
            // compares both at commit and drops a verdict from a connection
            // that has already been replaced.
            lifecycle.commit_disconnect(state_generation, incarnation);
        });
        Ok(AttachOutcome {
            attachment_epoch,
            read_write,
    ...
    pub async fn reconnect_session(&self, session_id: &str) -> Result<RemoteSessionLifecycle> {
        ...
        lifecycle.commit_reconnecting();
        match self.resume_once(&remote).await {
    ...
    async fn resume_once(&self, remote: &RemoteSession) -> Result<(), AttachFailure> {
        ...
        let sessions = list_sessions_on(&host).await?;
        let entry = sessions
            .iter()
        ...
        let local_view_ids = lifecycle.local_view_ids();
        if local_view_ids.is_empty() {
            lifecycle.commit_suspended();
            return Ok(());
        }
        ...
        for local_view_id in &local_view_ids {
            match self.attach_view_attempt(remote, local_view_id).await {
                Ok(outcome) => {
                    lifecycle.commit_view_attached(
                        local_view_id,
                    ...
                    if synchronized.is_none() {
                        synchronized = Some(outcome.synchronized);
                    }
                    attached += 1;
                }
                Err(AttachFailure::Ended(reason)) => {
                ...
                Err(AttachFailure::Failed(error)) => {
                    lifecycle.commit_view_failed(local_view_id, format!("{error:#}"), true);
                    last_error = Some(error);
                }
            }
        }
        if attached == 0 {
            return Err(AttachFailure::Failed(last_error.unwrap_or_else(|| {
                anyhow::anyhow!("no remote view could be attached")
            })));
        }

        lifecycle.commit_synchronizing();
        if let Some(synchronized) = synchronized {
            await_first_snapshot(synchronized).await?;
        }
        lifecycle.commit_live();
        Ok(())
    }
    ...
    async fn fail_connection(&self, device_id: &str, host: &Arc<RemoteHostConnection>) {
        ...
        for (session_id, generation, incarnation) in affected {
            if let Some(remote) = replicas.get(&session_id) {
                remote.lifecycle.commit_disconnect(generation, incarnation);
            }
        }
    }
    ...
    loop {
        let device_id = tokio::select! {
            event = peers.recv() => match event {
                Ok(event) => peer_probe_candidate(&event),
                Err(broadcast::error::RecvError::Lagged(_)) => continue,
                Err(broadcast::error::RecvError::Closed) => break,
            },
            event = advertisements.recv() => match event {
                Ok(event) => advertisement_probe_candidate(&event, now_ms()),
                Err(broadcast::error::RecvError::Lagged(_)) => continue,
                Err(broadcast::error::RecvError::Closed) => break,
            },
        };
...
mod tests {
    ...
    /// Both endpoints share one close flag, matching QUIC: closing either side
    /// breaks the streams already open on it.
