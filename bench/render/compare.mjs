...
if (positional.length !== 2) {
  console.error("Usage: node bench/render/compare.mjs baseline.json candidate.json [--noise=3] [--json=path]");
  process.exit(1);
}
...
if (issues.length > 0) {
  ...
  console.error("Capture clean, equivalent runs before assessing the optimization.");
  process.exit(1);
}
const comparisons = compareReports(baseline, candidate, noise);

console.log(`Rendering benchmark comparison (lower is better, ${noise}% practical threshold)`);
...
for (const caseName of [...new Set(comparisons.map((comparison) => comparison.caseName))]) {
  ...
  for (const comparison of comparisons.filter((value) => value.caseName === caseName)) {
    ...
    const interval = comparison.confidenceInterval95.every((value) => value != null)
      ...
      : "[n/a]";
    console.log(
      `  ${comparison.assessment.padEnd(12)} ${comparison.label.padEnd(28)} ${String(comparison.baselineMedian?.toFixed(2) ?? "n/a").padStart(10)} → ${String(comparison.candidateMedian?.toFixed(2) ?? "n/a").padStart(10)}  ${delta.padStart(8)}  95% ${interval}`,
    );
  }
  console.log("");
}
